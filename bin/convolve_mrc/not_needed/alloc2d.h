...
// Alloc2D() is a function for allocating 3-dimensional arrays of data
...
void Alloc2D(Integer size[3], 
             Entry *paX,      // <--pointer to 1-D contiguous-memory array
             Entry ***paaX) { // <--pointer to 3-D multidimensional array
  if (! paX)
    return;
  ...
  *paaX = new Entry* [size[1]];
  ...
  // (The aafX[][] pointers should point to to the 
