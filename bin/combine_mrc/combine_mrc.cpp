...
int main(int argc, char **argv) {
  try {
    ...
    if ((tomo1.mrc_header.nvoxels[0] != tomo2.mrc_header.nvoxels[0]) ||
	(tomo1.mrc_header.nvoxels[1] != tomo2.mrc_header.nvoxels[1]) ||
	(tomo1.mrc_header.nvoxels[2] != tomo2.mrc_header.nvoxels[2]))
      throw string("Error: The size of the two input tomograms does not match.\n");

    // Optional: Now apply the threshold filters to each input tomogram
    if (settings.in1_use_thresholds)
      for (int iz=0; iz<tomo1.mrc_header.nvoxels[2]; iz++)
	for (int iy=0; iy<tomo1.mrc_header.nvoxels[1]; iy++)
	  for (int ix=0; ix<tomo1.mrc_header.nvoxels[0]; ix++)
	    tomo1.aaafDensity[iz][iy][ix] =
	      Threshold4(tomo1.aaafDensity[iz][iy][ix],
    ...
    if (settings.in2_use_thresholds)
      for (int iz=0; iz<tomo2.mrc_header.nvoxels[2]; iz++)
	for (int iy=0; iy<tomo2.mrc_header.nvoxels[1]; iy++)
	  for (int ix=0; ix<tomo2.mrc_header.nvoxels[0]; ix++)
	    tomo2.aaafDensity[iz][iy][ix] =
	      Threshold4(tomo2.aaafDensity[iz][iy][ix],
	...
	for (int iy=0; iy<tomo1.mrc_header.nvoxels[1]; iy++) 
	  for (int ix=0; ix<tomo1.mrc_header.nvoxels[0]; ix++) 
	    out_tomo.aaafDensity[iz][iy][ix] = 
	      tomo1.aaafDensity[iz][iy][ix] * tomo2.aaafDensity[iz][iy][ix];
    ...
    for (int iz=0; iz<out_tomo.mrc_header.nvoxels[2]; iz++)
	  ...
	  if (out_tomo.aaafDensity[iz][iy][ix] < 0)
	    out_tomo.aaafDensity[iz][iy][ix] = 0;
	  else if (out_tomo.aaafDensity[iz][iy][ix] > 1)
	    out_tomo.aaafDensity[iz][iy][ix] = 1;
